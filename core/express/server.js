  ...
  function addHandler(handlerConfiguration) {
    ...
    app[method](path, async (expressRequest, expressResponse) => {
      ...
      try {
        ...
        if (result.disableResponseEnvelope) {
          const hasRawBody = Object.prototype.hasOwnProperty.call(result, 'rawBody');
          const rawBody = hasRawBody ? result.rawBody : '';

          if (result.contentType) {
            expressResponse.type(result.contentType);
          }

          responseComponents.body = rawBody;

          if (
            result.contentType === 'application/json' ||
            (rawBody && typeof rawBody === 'object' && !Buffer.isBuffer(rawBody))
          ) {
            expressResponse.status(responseComponents.statusCode).json(rawBody);
          } else {
            expressResponse.status(responseComponents.statusCode).send(rawBody);
          }
        } else {
